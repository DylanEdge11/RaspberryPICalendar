...
export function createWeatherService({ fetchImpl = fetch, now = Date.now } = {}) {
  ...
  async function get(city) {
    ...
    if (old && now() - old.checked < 15 * 60 * 1000) return old.value;
    ...
    const work = (async () => {
      try {
        ...
        const value = { location, timezone: data.timezone, current: data.current, hourly: data.hourly, daily: data.daily, updated_at: new Date(now()).toISOString(), stale: false };
        ...
        cache.set(city, { checked: now() - 10 * 60 * 1000, value });
        return value;
